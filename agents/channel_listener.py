...
# FIX: Resolve channel entities dulu sebelum register handler.
# Tanpa ini, Telethon tidak bisa filter NewMessage by string username
# dan handler tidak akan pernah terpicu (log kosong).
resolved = []
for ch in SIGNAL_CHANNELS:
    try:
        entity = await client.get_entity(ch)
        resolved.append(entity)
        print(f"[Listener] ✅ Resolved: {ch} → {getattr(entity, 'title', ch)}")
    except Exception as e:
        print(f"[Listener] ⚠ Gagal resolve {ch}: {e}")

if not resolved:
    print("[Listener] ✗ Tidak ada channel yang berhasil di-resolve. Listener tidak aktif.")
    return
