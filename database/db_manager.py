...
FIXED: Proper update_prediction_outcome with two variants (by ID and by address).
"""
import aiosqlite
import json
import os
...
async def save_prediction(contract_address: str, prediction_type: str, predicted_multiplier: float,
                          safe_tp_multiplier: float, confidence: float, reasoning: str, ai_raw: str) -> Optional[int]:
    """Save prediction and return the prediction ID."""
    async with aiosqlite.connect(DB_PATH) as db:
...
async def update_prediction_outcome(prediction_id: int, actual_multiplier: float):
    """
    Update prediction outcome by prediction ID.
    Used by token_monitor at end of session.
    """
    async with aiosqlite.connect(DB_PATH) as db:
        cursor = await db.execute(
            "SELECT predicted_multiplier FROM predictions WHERE id = ?", 
            (prediction_id,)
        )
        pred = await cursor.fetchone()
        if not pred:
            print(f"[DB] No prediction found with id={prediction_id}")
            return

        predicted_mult = pred[0]
        was_correct = 1 if actual_multiplier >= (predicted_mult * 0.7) else 0

        await db.execute("""
            UPDATE predictions 
            SET resolved_at = ?, actual_multiplier = ?, was_correct = ?
            WHERE id = ?
        """, (
        ...
        await db.commit()
        print(f"[DB] Prediction {prediction_id} resolved: actual={actual_multiplier}x, correct={was_correct}")

async def update_prediction_outcome_by_address(contract_address: str, actual_multiplier: float):
    """
    Update prediction outcome by contract address (for update messages).
    Finds the most recent unresolved prediction for this token.
    """
    async with aiosqlite.connect(DB_PATH) as db:
        cursor = await db.execute("""
        ...
        pred = await cursor.fetchone()
        if not pred:
            print(f"[DB] No unresolved prediction found for {contract_address[:8]}...")
            return
        ...
        await db.commit()
        print(f"[DB] Prediction for {contract_address[:8]}... resolved: actual={actual_multiplier}x, correct={was_correct}")
